...
int main()
{
	...
	while (true)
	{
		int*** matrix;							//zmienna przechowujaca macierze
		matrix = new int** [3];						//okreslenie ilosci wymiarow (macierzy)
		...
		/*CASE1*/ case 1:
		{
			...
			if (det_option == 1)
			{
				...
				for (int i = 0; i < row[0]; i++) { WK[i] = i; }                   // wypelniamy go numerami kolumn
		...
		/*CASE3*/ case 3:
		{
			...
			if (square_statement(row[2], column[2])) { //if sprawdza czy macierz jest kwadratowa
				...
				if (det_option == 1)
				{
					...
					for (int i = 0; i < row[2]; i++) { WK[i] = i; }                   // wypelniamy go numerami kolumn
